...
auto run_observation_intent_control_contract() -> int {
    ...
    const auto intent_request = [](std::string_view id,
                                   std::string_view method,
                                   std::string_view payload,
                                   std::optional<std::string_view> idempotency_key = std::nullopt) {
        return make_request(
    ...
    auto shared_sessions = std::shared_ptr<glove::control::session_registry>{std::move(*registry)};
    ...
    auto keyed_page = (*intent_protocol)
                          ->handle_frame(
                              intent_request(
                                  "intent-page-keyed",
                                  "page_observation_intents",
    ...
    auto missing_limit = (*intent_protocol)
                             ->handle_frame(
                                 intent_request(
                                     "intent-page-missing-limit",
                                     "page_observation_intents",
                                     "{\"after_sequence\":0}"
                                 ),
                                 50'000
    ...
    auto unknown_page_field = (*intent_protocol)
                                  ->handle_frame(
                                      intent_request(
                                          "intent-page-unknown",
                                          "page_observation_intents",
    ...
    auto zero_limit = (*intent_protocol)
                          ->handle_frame(
                              intent_request(
                                  "intent-page-zero",
                                  "page_observation_intents",
        ...
        (*intent_protocol)
            ->handle_frame(
                intent_request(
                    ...
                    "{\"session_id\":\"missing\",\"channel_generation\":1,\"intent_id\":\"intent-"
                    "1\","
                    "\"intent_digest\":\"" +
                        std::string(64, 'a') +
    ...
    auto reserved =
        shared_sessions->reserve_start(authorization, "reserve-intent-session", start_ms + 2U);
    ...
    auto first_page_frame = (*intent_protocol)
                                ->handle_frame(
                                    intent_request(
                                        "intent-page-first",
                                        "page_observation_intents",
    ...
    const std::string disposition_payload =
        ...
        enqueued->intent_digest + "\",\"disposition\":\"accepted\",\"decided_at_ms\":50020}";
    auto accepted_frame = (*intent_protocol)
                              ->handle_frame(
                                  intent_request(
    ...
    auto accepted_replay = (*intent_protocol)
                               ->handle_frame(
                                   intent_request(
    ...
    auto accepted_restart_replay = (*restarted_protocol)
                                       ->handle_frame(
                                           intent_request(
    ...
    auto missing_intent_payload =
        ...
        std::string(64, 'b') + "\",\"disposition\":\"accepted\",\"decided_at_ms\":50023}";
    auto missing_intent = (*intent_protocol)
                              ->handle_frame(
                                  intent_request(
    ...
    auto expired_disposition_payload =
        ...
        expiring->intent_digest + "\",\"disposition\":\"accepted\",\"decided_at_ms\":50030}";
    auto expired_disposition = (*intent_protocol)
                                   ->handle_frame(
                                       intent_request(
    ...
    auto expired_page = (*intent_protocol)
                            ->handle_frame(
                                intent_request(
    ...
    const std::string expire_payload =
        std::string{"{\"session_id\":\""} + session_id +
        "\",\"channel_generation\":1,\"intent_id\":\"intent-expired\",\"intent_digest\":\"" +
        expiring->intent_digest + "\",\"disposition\":\"expired\",\"decided_at_ms\":50020}";
    auto expire_frame = (*intent_protocol)
                            ->handle_frame(
                                intent_request(
