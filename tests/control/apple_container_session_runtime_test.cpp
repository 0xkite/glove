...
public:
    ...
    auto start() -> bool {
        ...
        if (descriptor_ < 0 ||
            ::bind(
                descriptor_, reinterpret_cast<::sockaddr*>(&address), sizeof(address)
            ) != 0 ||
        ...
        if (::getsockname(
                descriptor_, reinterpret_cast<::sockaddr*>(&bound), &size
            ) != 0) {
            return false;
        }
...
auto run() -> int {
    ...
    const char* closure_digest =
        std::getenv("GLOVE_APPLE_CONTAINER_HARNESS_CLOSURE_DIGEST");
    ...
    const std::filesystem::path cli =
        configured_cli != nullptr && *configured_cli != '\0' ? configured_cli
    ...
    const auto adapter_digest =
        glove::supervisor::runtime_launch_template_digest(launch);
    ...
    const auto plan =
        ...
        (egress_probe ? "glove-egress-probe" : managed_closure ? "claude-code" : "probe") +
        R"(","runtime_template_id":"apple-probe","adapter_command_digest":")" +
        *adapter_digest +
    ...
    const std::string expected_ready =
        egress_probe ? "GLOVE_EGRESS_OK" : managed_closure ? "2.1.220" : "APPLE_READY";
