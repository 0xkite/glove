...
constexpr std::size_t max_replay_records = 1'024U;
...
auto constant_time_equal(std::string_view left, std::string_view right) noexcept -> bool {
    ...
    for (std::size_t index = 0; index < maximum; ++index) {
        const auto left_byte =
            static_cast<unsigned char>(index < left.size() ? left[index] : 0);
...
auto set_io_timeout(int descriptor, std::uint64_t timeout_ms)
...
auto read_exact(int descriptor, void* output, std::size_t size) -> std::expected<void, std::string> {
...
struct observation_intent_unix_server::implementation {
    ...
    std::unordered_map<std::string, replay_record> replays;
    ...
    ~implementation() {
        ...
        if (::lstat(config.socket_path.c_str(), &metadata) == 0 &&
            S_ISSOCK(metadata.st_mode) && metadata.st_dev == socket_device &&
            metadata.st_ino == socket_inode) {
    ...
    auto handle(std::string_view frame, std::uint64_t now_ms)
        ...
        if (frame.empty() || frame.size() > max_observation_frame_bytes ||
            glz::read<strict_complete_read_options>(request, frame) || request.schema_version != 1U) {
        ...
        if (const auto replay = replays.find(request.body.intent_id); replay != replays.end()) {
            if (replay->second.body != request.body) {
                return encode(enqueue_error_v1{.code = "idempotency_conflict"});
            }
            return encode(replay->second.response);
        }
        if (replays.size() >= max_replay_records) {
            return encode(enqueue_error_v1{.code = "capacity_exhausted"});
        }
        const auto ttl_ceiling = now_ms > std::numeric_limits<std::uint64_t>::max() - max_intent_ttl_ms
        ...
        const auto intent_id = request.body.intent_id;
        replays.emplace(
            intent_id, replay_record{.body = std::move(request.body), .response = response}
        );
...
auto observation_intent_unix_server::create(observation_intent_unix_server_config config)
    ...
    if (!config.sessions || !valid_identifier(config.session_id) ||
        !valid_hex(config.controller_plan_digest) || !valid_hex(config.profile_digest) ||
        !valid_hex(config.projection_digest) || config.policy_revision == 0U ||
        !valid_identifier(config.service_channel_id) || config.channel_generation == 0U ||
        config.session_expires_at_ms == 0U || !valid_hex(config.channel_token) ||
        config.io_timeout_ms == 0U || config.io_timeout_ms > max_io_timeout_ms) {
        wipe(config.channel_token);
    ...
    struct stat metadata {};
    if (::lstat(state->config.socket_path.c_str(), &metadata) != 0 ||
    ...
    return std::make_unique<observation_intent_unix_server>(
...
auto observation_intent_unix_server::serve_one_for(std::uint64_t accept_timeout_ms)
    ...
    const int timeout = accept_timeout_ms > static_cast<std::uint64_t>(std::numeric_limits<int>::max())
                            ? std::numeric_limits<int>::max()
                            : static_cast<int>(accept_timeout_ms);
    ::pollfd event{.fd = state_->listener.get(), .events = POLLIN, .revents = 0};
    int ready = 0;
    do {
    ...
    if (ready < 0 || (event.revents & POLLIN) == 0) {
        return std::unexpected(system_error("wait for observation connection"));
    }
#if defined(SOCK_CLOEXEC)
    unique_fd client{::accept4(state_->listener.get(), nullptr, nullptr, SOCK_CLOEXEC)};
#else
    unique_fd client{::accept(state_->listener.get(), nullptr, nullptr)};
#endif
    if (client.get() < 0) {
        return std::unexpected(system_error("accept observation connection"));
    }
#if !defined(SOCK_CLOEXEC)
    if (::fcntl(client.get(), F_SETFD, FD_CLOEXEC) != 0) {
        return std::unexpected(system_error("protect observation client descriptor"));
    }
#endif
