...
| Local services   | Linux owner-`0600` Unix endpoints under descriptor-pinned owner-`0700` parents; before/after endpoint drift checks; descriptor-ancestry exclusion from path grants; runtime allowlists; compatibility pathname projection or adapter-selected preconnected inherited streams; exact child-FD allowlist and closure; peer-UID checks; bounded sequential G2 exchanges; pending-before-release audit; poison-on-failure teardown; inherited-only exact adapter/runtime/catalog capability proof |
