...
| Local services   | Linux-only owner-`0600` Unix sockets under descriptor-pinned owner-`0700` parents; before/after endpoint drift checks; descriptor-ancestry exclusion from path grants; runtime allowlists; fixed guest directory; peer-UID checks; one opaque bounded G2 exchange; pending-before-release audit; inode-safe joined teardown; exact adapter/runtime/catalog capability proof |
