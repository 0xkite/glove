...
| Wallet status bridge is construction-only | Offline authority and socketpair fakes could be mistaken for live wallet mediation, or independently changing authority sources could produce a mixed snapshot. | Keep production construction and global capability advertisement absent until protected authorities and separately reviewed child-FD mapping are composed. Require verified owner-only Unix-stream endpoints, CLOEXEC-by-default transfer, one deadline and plan generation across snapshots, exact teardown on partial/error/cancel paths, status-only output, and no fallback to generic Sage, MCP, shell, or wallet routes. |
| Synthetic Pi status projection is mistaken for activation | A test-only Pi-shaped descriptor could be treated as proof that the contained process can safely discover or invoke wallet status. | Keep the adapter private, disabled, fake-channel-only, and absent from kernel/native/Pi registries. Re-attest every list and call, disappear on any untrusted state, expose no host identifiers or errors, and require separate child mapping, lifetime, and activation review. |
