...
| Wallet status bridge is construction-only | Offline authority fakes could be mistaken for live wallet mediation, or independently changing authority sources could produce a mixed snapshot. | Keep global capability advertisement absent until protected plan/session/policy, audit/journal, containment, adapter, stable-peer status, and inherited-descriptor implementations are composed and independently tested. Require one deadline and plan generation across snapshots, sandwich them with a plan re-read, expose status only, and never fall back to generic Sage, MCP, shell, or wallet routes. |
